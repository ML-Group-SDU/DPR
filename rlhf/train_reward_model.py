...
from reward_model import RewardModel, CNNRewardModel, TransformerRewardModel, DdpmRewardModel, CondDdpmRewardModel
import logger
import random
import torch
import pickle
...
def train(cfg):
    ...
    cfg_data = parse_cfg(Path().cwd() / __CONFIG__)
    # train reward model
    if cfg.modality == "state":
        if cfg.structure == 'mlp':
        ...
        elif "DPR" == cfg.structure:
            max_action = float(gym_env.action_space.high[0])
            reward_model = DdpmRewardModel(
                cfg.env, observation_dim, action_dim, ensemble_size=cfg.ensemble_size, lr=cfg.disc_lr,
            ...
            pref_dataset = load_queries_with_indices(
                dataset, cfg.num_query, cfg.len_query, saved_indices=[human_indices_1, human_indices_2],
                saved_labels=label_file, scripted_teacher=cfg.fake_label, modality=cfg.modality)
            train_metrics = reward_model.train_rm(n_epochs=cfg.n_epochs, pref_dataset=pref_dataset,
                                                  data_size=pref_dataset["observations"].shape[0],
                                                  batch_size=cfg.batch_size, loss_type=cfg.loss_type)
        elif "C-DPR" == cfg.structure:
            reward_model = CondDdpmRewardModel(
                cfg.env, observation_dim, action_dim, ensemble_size=cfg.ensemble_size, lr=cfg.disc_lr,
                n_timesteps=cfg.n_steps,
                num_units=cfg.num_units,
                device=cfg.device,
                disc_momentum=cfg.disc_momentum,
                lable_dim=cfg.lable_dim,
                discrim_depth=cfg.discrim_depth,
