...
from DPR.diffusion.helpers import (cosine_beta_schedule,
                                         linear_beta_schedule,
                                         vp_beta_schedule,
                                         extract,
                                         Losses)
from DPR.diffusion.utils import Progress, Silent, normal_kl, \
...
class Diffusion(nn.Module):
    ...
    def compute_loss(self, x_start, grad=False):
        device = x_start.device
        batch_size = x_start.shape[0]
        ...
        vb = torch.stack(vb, dim=1)
        loss = vb.sum(dim=1) / (self.n_timesteps)
        loss = loss.sum(dim=1) / (x_start.shape[1])
        return loss