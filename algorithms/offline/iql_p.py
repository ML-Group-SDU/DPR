...
from rlhf.cfg import parse_cfg
from rlhf.train_reward_model import __CONFIG__
from rlhf.reward_model import DdpmRewardModel, CondDdpmRewardModel
...
class TrainConfig:
    ...
    seed: int = 99  # Sets Gym, PyTorch and Numpy seeds
...
def train(config: TrainConfig):
    ...
    elif config.reward_model_type == 'DPR':
        max_action = float(env.action_space.high[0])
        data = str(config.env).split('-')[0]
        cfg_data = parse_cfg(Path().cwd() / 'rlhf' / __CONFIG__)

        reward_model = DdpmRewardModel(
            config.env, state_dim, action_dim, ensemble_size=3, lr=0.0006,
            max_action=max_action,
            n_timesteps=config.n_steps,
            beta_schedule=cfg_data.disc_ddpm_beta_schedule,
            disc_hid_dim=cfg_data.disc_hid_dim,
            device="cuda:0",
            disc_momentum=cfg_data.disc_momentum,
            logger=None
        )
    elif config.reward_model_type == 'C-DPR':
        max_action = float(env.action_space.high[0])
        data = str(config.env).split('-')[0]
        cfg_data = parse_cfg(Path().cwd() / 'rlhf' / __CONFIG__)

        reward_model = CondDdpmRewardModel(
            config.env, state_dim, action_dim, ensemble_size=3, lr=0.0003,
            n_timesteps=config.n_steps,
            num_units=cfg_data.num_units,
            device=cfg_data.device,
            disc_momentum=cfg_data.disc_momentum,
            lable_dim=cfg_data.lable_dim,
            discrim_depth=cfg_data.discrim_depth,
            logger=None,
            training=False,
        )
    ...
    for t in range(int(config.max_timesteps)):
        ...
        if (t + 1) % config.eval_freq == 0:
            ...
            evaluations.append((t + 1, normalized_eval_score))
            print("---------------------------------------")
            print(
            ...
            # np.savetxt(os.path.join(config.checkpoints_path, "eval_score.csv"), evaluations, fmt=['%d', '%.4f'])
    ...
    print(
        f"D4RL score: {normalized_eval_score:.3f}, "
        f"Envs: {config.env}, Seed: {config.seed}"
    )
